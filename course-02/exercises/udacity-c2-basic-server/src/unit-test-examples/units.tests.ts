...
describe('concat', () => {
  ...
  it('should concat two strings', () => {
    const result = concat('Get',' up!');
    expect(result).to.equal('Get up!');
  });

  it('should throw an error if parameter "a" is empty', () => {
    expect(()=>{ concat('Get','') }).to.throw('Empty parameter')
  });

  it('should throw an error if parameter "b" is empty', () => {
    expect(()=>{ concat('','up!') }).to.throw('Empty parameter')
  });
